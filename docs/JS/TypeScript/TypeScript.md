...
Utility types - types that can be created from base
...
type ToDoUpdateOrCreate = Omit<ToDo, 'id'>; // Contains fields title, description, isDone

// Select only specified keys
type ToDoUpdateOrCreate = Pick<ToDo, 'id' | 'title'>; // Contains fields id and title
```
